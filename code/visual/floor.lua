...
local objs = {}
local focus = {}
local focus_obj = ant.prefab("/asset/wall/floor.glb", { x = 0, y = 0, z = 0.1 })
...
local function enable_floor(x, y)
	...
	if not obj then
		...
		if obj then
			...
			obj = ant.prefab("/asset/wall/floor.glb", { x = x, y = y, z = 0, material = { color = 0xffffff } })
		end
		objs[index] = obj
	end
end
...
local function color_floor(x, y, color)
	...
	if not obj then
		...
		if obj then
			...
			obj = ant.prefab("/asset/wall/floor.glb", { x = x, y = y, z = 0.1, material = { color = color }})
		end
		focus[index] = obj
	end
end
